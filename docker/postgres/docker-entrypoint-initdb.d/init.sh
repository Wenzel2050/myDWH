...
#
#	Prepare Redash database
#
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" <<-EOSQL
  CREATE USER ${REDASHDB_USER} WITH PASSWORD '${REDASHDB_PASSWORD}';
  CREATE DATABASE ${REDASHDB_NAME};
  GRANT ALL PRIVILEGES ON DATABASE ${REDASHDB_NAME} TO ${REDASHDB_USER};
EOSQL

psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" -d "${REDASHDB_NAME}" <<-EOSQL
   GRANT ALL ON SCHEMA public TO ${REDASHDB_USER};
EOSQL
